...
segment ("sdram0") uint8_t guardado[BUFFER_SIZE_CONV];
...
void salirPorDAC()
{
	...
	if(bIsBufAvailable && !Result)
	{
		Result = (uint32_t) adi_ad1854_GetTxBuffer (hAd1854Dac, &pDacBuf);

		if(!Result)
		{
			memcpy(pDacBuf, guardado, BUFFER_SIZE_CONV);
...
void obtenerEntradaADC()
{
	...
	else if(bIsBufAvailable)
	{
		Result = (uint32_t) adi_ad1871_GetRxBuffer (hAd1871Adc, &pAdcBuf);

		if(!Result)
		{
			memcpy(guardado, pAdcBuf, BUFFER_SIZE_CONV);
